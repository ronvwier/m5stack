""""
  Library for the Maxim MAX30100 pulse oximetry system on M5Stack, ported from Pi
...
class MAX30100(object):
    ...
    def set_led_current(self, led_current_red=11.0, led_current_ir=11.0):
        ...
        self.i2c.write_u8(LED_CONFIG, (led_current_red << 4) | led_current_ir)

    def set_mode(self, mode):
        reg = self.i2c.read_u8(MODE_CONFIG)
        self.i2c.write_u8(MODE_CONFIG, reg & 0x74) # mask the SHDN bit
        self.i2c.write_u8(MODE_CONFIG, reg | mode)

    def set_spo_config(self, sample_rate=100, pulse_width=1600):
        reg = self.i2c.read_u8(SPO2_CONFIG)
        reg = reg & 0xFC  # Set LED pulsewidth to 00
        self.i2c.write_u8(SPO2_CONFIG, reg | pulse_width)
    ...
    def enable_interrupt(self, interrupt_type):
        self.i2c.write_u8(INT_ENABLE, (interrupt_type + 1)<<4)
        self.i2c.read_u8(INT_STATUS)

    def get_number_of_samples(self):
        write_ptr = self.i2c.read_u8(FIFO_WR_PTR)
        read_ptr = self.i2c.read_u8(FIFO_RD_PTR)
        return abs(16+write_ptr - read_ptr) % 16

    def read_sensor(self):
        bytes = [0,1,2,3]
        for i in range(0, 4):
            bytes[i] = self.i2c.read_u8(FIFO_DATA) # read 4 bytes (order?)
        # Add latest values.
    ...
    def shutdown(self):
        reg = self.i2c.read_u8(MODE_CONFIG)
        self.i2c.write_u8(MODE_CONFIG, reg | 0x80)

    def reset(self):
        reg = self.i2c.read_u8(MODE_CONFIG)
        self.i2c.write_u8(MODE_CONFIG, reg | 0x40)

    def refresh_temperature(self):
        reg = self.i2c.read_u8(MODE_CONFIG)
        self.i2c.write_u8(MODE_CONFIG, reg | (1 << 3))

    def get_temperature(self):
#        intg = _twos_complement(self.i2c.read_u8(TEMP_INTG))
        intg = self.i2c.read_u8(TEMP_INTG)
        frac = self.i2c.read_u8(TEMP_FRAC)
        return intg + (frac * 0.0625)

    def get_rev_id(self):
        return self.i2c.read_u8(REV_ID)

    def get_part_id(self):
        return self.i2c.read_u8(PART_ID)

    def get_registers(self):
        return {
            "INT_STATUS": self.i2c.read_u8(INT_STATUS),
            "INT_ENABLE": self.i2c.read_u8(INT_ENABLE),
            "FIFO_WR_PTR": self.i2c.read_u8(FIFO_WR_PTR),
            "OVRFLOW_CTR": self.i2c.read_u8(OVRFLOW_CTR),
            "FIFO_RD_PTR": self.i2c.read_u8(FIFO_RD_PTR),
            "FIFO_DATA": self.i2c.read_u8(FIFO_DATA),
            "MODE_CONFIG": self.i2c.read_u8(MODE_CONFIG),
            "SPO2_CONFIG": self.i2c.read_u8(SPO2_CONFIG),
            "LED_CONFIG": self.i2c.read_u8(LED_CONFIG),
            "TEMP_INTG": self.i2c.read_u8(TEMP_INTG),
            "TEMP_FRAC": self.i2c.read_u8(TEMP_FRAC),
            "REV_ID": self.i2c.read_u8(REV_ID),
            "PART_ID": self.i2c.read_u8(PART_ID),
        }
...
devicesList = M5TextBox(1, 81, "Devices on I2C bus", lcd.FONT_DejaVu18,0xFFFFFF, rotate=0)
...
i2c0 = i2c_bus.easyI2C(i2c_bus.PORTA, 0x57)
statusLabel.setText('Port open')
lijst = i2c0.scan()
devicesList.setText(str(lijst))

hr = MAX30100(i2c0, mode=MODE_SPO2)
hr.read_sensor()
hr.refresh_temperature()
hr.enable_spo2()
statusLabel.setText(str(hr.buffer_red))
devicesList.setText(str(hr.get_registers()))